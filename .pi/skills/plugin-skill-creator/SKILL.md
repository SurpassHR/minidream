...
- **不要**在正文生成 YAML/TOML/JSON 配置；唯一允许的 YAML 是输出模板要求的 frontmatter。
...
8. **输出格式**：只输出 markdown 本身，不要任何解释、开场白或结尾语。输出的第一行必须是 `---`（YAML frontmatter 起始），最后一行必须是 `## 使用规则` 下的具体规则内容。不要生成 `response` frontmatter 或 `## 回复协议` 章节。
