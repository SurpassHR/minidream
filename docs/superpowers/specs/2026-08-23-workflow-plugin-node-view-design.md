...
- 勾选 = 加入 LLM 上下文（`llm: true`）：参数出现在表单视图和 MCP `workflow.list`，由 LLM 控制和修改。
- 所有可选 combo widget 都可以在节点视图直接配置固定值：修改 combo 值即生成/更新参数，未勾选时该参数带 `llm: false`，只参与运行时注入，不暴露给 LLM。
- 取消勾选 combo 参数只退出 LLM 上下文（保留已固定值，无需确认）；普通参数无独立配置，取消勾选整项删除并弹出确认。
- 已固定（`llm: false`）的 combo 参数可通过 combo 控件旁的“×”删除，恢复模板默认值。
- 现有插件首次进入节点视图时，只恢复当前 manifest 中已有的参数状态，其他字段默认不勾选。
- 取消普通已配置参数时必须弹出确认，确认后丢弃该参数的 description 和配置值。
- 重新勾选已取消的参数时视为新参数，接受从当前 workflow 字段推导出的默认配置，不恢复历史 description 和参数值。
- 节点视图和表单视图共享同一个本地 draft，切换视图不丢失修改。
- 未勾选（`llm: false`）参数不出现在表单视图，也不出现在 MCP `workflow.list`。
...
- MCP `workflow.list` 只暴露当前 params 中未 hidden 且加入 LLM 上下文（`llm !== false`）的参数的紧凑摘要（id/label/type），不输出默认值与范围等细节。
