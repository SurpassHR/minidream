...
function customSkillMarkerPath(id: string, root: string = PLUGIN_SKILLS_DIR): string {
  return `${pluginSkillPath(id, root)}.custom`;
}

function writeMarker(file: string): void {
  const tmp = `${file}.tmp`;
  writeFileSync(tmp, 'custom\n', 'utf8');
  renameSync(tmp, file);
}
...
export function writePluginSkill(spec: WorkflowSpec, root: string = PLUGIN_SKILLS_DIR): string {
  ...
  renameSync(tmp, file);
  const marker = customSkillMarkerPath(spec.id, root);
  if (existsSync(marker)) rmSync(marker, { force: true });
  return content;
}

/** 原子写入自定义内容（手工编辑或 LLM 生成），并记录自定义版本。 */
export function writeCustomSkill(id: string, content: string, root: string = PLUGIN_SKILLS_DIR): void {
  ...
  writeFileSync(tmp, content, 'utf8');
  renameSync(tmp, file);
  writeMarker(customSkillMarkerPath(id, root));
}
...
export function syncPluginSkill(spec: WorkflowSpec, root: string = PLUGIN_SKILLS_DIR): void {
  const file = pluginSkillPath(spec.id, root);
  const marker = customSkillMarkerPath(spec.id, root);
  if (!existsSync(file) || (!existsSync(marker) && isAutoGeneratedSkill(readFileSync(file, 'utf8')))) {
...
export function deletePluginSkill(id: string, root: string = PLUGIN_SKILLS_DIR): void {
  ...
  const marker = customSkillMarkerPath(id, root);
  if (existsSync(marker)) rmSync(marker, { force: true });
}
