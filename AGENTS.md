...
- 插件 Skill 的 `response` frontmatter 仅作为旧版兼容回退；新的机器协议位于 `.pi/skills/<plugin-id>/response.json`，由回复协议编辑器维护。
